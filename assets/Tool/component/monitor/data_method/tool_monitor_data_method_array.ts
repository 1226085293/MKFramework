	...
	class array_extend<CT> extends Array<CT> {
		...
		async init(init_: array_init_config): Promise<void> {
			...
			if (this._item_view_type) {
				...
				await mk.uiManage.regis(this._item_view_type, this._init_data.item, this._ui_regis_target, {
					repeat_b: true,
					pool_init_fill_n: 8,
			...
			await this._task_pipeline.add(async () => {
				...
				if (this._item_view_type) {
					while (this._init_data.root.children.length) {
						await mk.uiManage.close(this._init_data.root.children[0]);
					}

					await mk.uiManage.unregis(this._item_view_type);
				}
				// 节点
				else {
		...
		private async _create_item(init_data_?: any): Promise<cc.Node | null> {
			...
			if (this._item_view_type) {
				if (this._ui_regis_target.valid_b) {
					const view_comp = await mk.uiManage.open(this._item_view_type, { init: init_data_ });
			...
			.on(
				...
				async () => {
					...
					if (layout_node?.isValid) {
						...
						await mk.uiManage.close(item_node);
					}
				},
				target_
			)
