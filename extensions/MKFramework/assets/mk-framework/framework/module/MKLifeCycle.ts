...
import { mkAudio, mk_audio_ } from "../Audio/MKAudioExport";
import mk_release, { mk_release_ } from "../mk_release";
import { mk_asset_ } from "../resources/mk_asset";
import GlobalConfig from "../../Config/GlobalConfig";
const uiManage = mk_dynamic_module.default(import("../mk_ui_manage"));
 ...
 */
@ccclass
export class MKLifeCycle extends MKLayer implements mk_asset_.type_follow_release_object {
	 ...
	 * 模块关闭后自动清理事件
	 */
	eventTargetList: { targetOff?(target: any): any; target_off?(target: any): any }[] = [];
	...
	/** onLoad 任务 */
	protected _onLoadTask = new mk_status_task(false);
	/** create 任务 */
	protected _createTask = new mk_status_task(false);
	/** open 任务 */
	protected _openTask = new mk_status_task(false);
	/** 运行状态 */
	 ...
	 * @internal
	 */
	protected _releaseManage = new mk_release();
	/**
	 * 重置 data
	 * @remarks
	...
	protected async onLoad(): Promise<void> {
		...
		attrKeyStrList.forEach((vStr) => {
			...
			// 初始化音频单元
			if (this[nameStr] instanceof mk_audio_.PrivateUnit) {
	...
	protected async lateClose?(): Promise<void> {
		...
		// 释放资源
		await this._releaseManage.release_all();
		// 重置数据
	...
	followRelease<T = mk_release_.type_release_param_type & mk_audio_.PrivateUnit>(object_: T): T {
		if (!object_) {
			return object_;
		}
		...
		// 添加释放对象
		if (object_ instanceof mk_audio_.PrivateUnit) {
			if (object_.clip) {
				// 如果模块已经关闭则直接释放
	...
	cancelRelease<T = mk_release_.type_release_param_type & mk_audio_.PrivateUnit>(object_: T): void {
		if (!object_) {
			return;
		}
	...
	async _close(config_?: _MKLifeCycle.CloseConfig): Promise<void> {
		...
		// 等待模块 open 完成
		if (!config.isForce && !this._openTask.finish_b) {
