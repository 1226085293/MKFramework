	...
	async add<T extends string | string[], T2 extends true | false = false>(
		...
		if (config_?.isDir) {
			for (const vStr of urlStrList) {
				const assetList = await mk_asset.get_dir(vStr, cc.AudioClip, target_, config_.loadConfig as any);
...
export namespace MKAudioBase_ {
	...
	export interface Unit {
		...
		/** 事件对象 */
		readonly event: mk_event_target<EventProtocol>;
		/** 音频类型 */
	...
	export interface AddConfig<T extends boolean> {
		...
		isDir?: T;
		/** 加载配置 */
		loadConfig?: mk_asset_.get_config<cc.AudioClip>;
	}
	...
	export abstract class PrivateUnit {
		...
		/* --------------- public --------------- */
		/** 事件对象 */
		_event?: mk_event_target<EventProtocol>;
		/** 分组 */
		groupNumList: number[] = [];
		/**
		 * 当前停止分组
		...
		/** 事件对象 */
		get event(): mk_event_target<EventProtocol> {
			return this._event ?? (this._event = new mk_event_target<EventProtocol>());
		}
