...
import mkBundle from "../resources/mk_bundle";
...
class MKGuideManage {
	constructor(init_: MKGuideManage_.InitConfig) {
		// 初始化数据
		this._log = new mk_logger(init_.nameStr ?? "guide_manage");
	...
	/* --------------- public --------------- */
	/** 事件 */
	event = new mk_event_target<MKGuideManage_.EventProtocol>();
	/** 步骤表 */
	...
	/** 任务管线 */
	private _taskPipeline = new mk_task_pipeline();
	/** 步骤预加载任务表 */
