 ...
 * - 报错日志 http 上传
 */
class mk_logger extends MKInstanceBase {
	constructor(name_s_: string) {
		...
		// 替换日志函数
		if (mk_logger._config.logObjectType === GlobalConfig.Log.LogObjectType.Console) {
			this.debug = this._log_func_tab[mk_logger._config.logObjectType].debug;
			this.log = this._log_func_tab[mk_logger._config.logObjectType].log;
			this.warn = this._log_func_tab[mk_logger._config.logObjectType].warn;
			this.error = this._log_func_tab[mk_logger._config.logObjectType].error;
		}
		...
		if (!mk_logger._init_b) {
			...
			const upload_f = (...args_as: any[]): void => {
				// 添加日志缓存
				mk_logger._add_log_cache(GlobalConfig.Log.Level.Error, mkLog._get_log_head(GlobalConfig.Log.Level.Error, true), args_as);

				// 上传错误日志
				if (mk_logger._config.errorUploadAddrStr) {
					mk_http.post(mk_logger._config.errorUploadAddrStr, {
				...
				// 错误处理
				mk_logger._config.errorHandlingFunc?.(...args_as);
			};
	 ...
	 * @returns
	 */
	private static _add_log_cache(level_: GlobalConfig.Log.Level, head_s_: string, ...args_as_: any[]): void {
		if (!args_as_?.length || mk_logger._config.cacheRowNum <= 0) {
			return;
		}
		...
		// 超出缓存删除顶部日志
		if (mk_logger._cache_ss.length > mk_logger._config.cacheRowNum) {
	...
	debug(...args_as_: any[]): void {
		this._log(GlobalConfig.Log.Level.Debug, ...args_as_);
	}

	log(...args_as_: any[]): void {
		this._log(GlobalConfig.Log.Level.Log, ...args_as_);
	}

	warn(...args_as_: any[]): void {
		this._log(GlobalConfig.Log.Level.Warn, ...args_as_);
	}

	error(...args_as_: any[]): void {
		this._log(GlobalConfig.Log.Level.Error, ...args_as_);
	}
	...
	time_start(name_s_: string, ...args_as_: any[]): void {
		...
		if (args_as_?.length) {
			this._log(GlobalConfig.Log.Level.Log, name_s_, ...args_as_);
		}
	}
	...
	time_log(name_s_: string, ...args_as_: any[]): void {
		...
		if (args_as_?.length) {
			this._log(GlobalConfig.Log.Level.Log, name_s_, ...args_as_, `耗时：${(curr_time_ms_n - time_log.last_time_ms_n) / 1000}s`);
		}
	...
	time_end(name_s_: string, ...args_as_: any[]): void {
		...
		this._log(GlobalConfig.Log.Level.Log, name_s_, ...args_as_, `总耗时：${(Date.now() - time_log.start_time_ms_n) / 1000}s`);
		this._time_map.delete(name_s_);
	}

	/** 日志头 */
	private _get_log_head(level_: GlobalConfig.Log.Level, time_b_ = true): string {
		const date = new Date();
		...
		if (time_b_) {
			...
			return `${this._name_s} <${GlobalConfig.Log.Level[level_]}> [${time_s}]：`;
		} else {
			return `${this._name_s} <${GlobalConfig.Log.Level[level_]}>：`;
		}
	}

	private _log(level_: GlobalConfig.Log.Level, ...args_as_: any[]): void {
		// 打印等级限制
		if (!(mk_logger._config.levelNum & level_)) {
			return;
		}
		...
		const logger = this._log_func_tab[mk_logger._config.logObjectType];

		// 更新缓存
		mk_logger._add_log_cache(level_, head_s, ...args_as_);
		// 打印日志
		logger[GlobalConfig.Log.Level[level_]].call(logger.target, head_s, ...args_as_);
	}
}

export const mkLog = mk_logger.instance("default");
